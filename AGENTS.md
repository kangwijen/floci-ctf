...
| ContainerSpec privileged / docker.sock binds (non-operator) | Closed | `ContainerSpecHardening` rejects privileged and docker.sock binds/mounts at `ContainerLifecycleManager.create` unless `operatorManaged` (EC2, EKS/k3s). Regressions: `ContainerSpecHardeningTest`. Residual: Compose still mounts host RW `docker.sock` into Floci (D.2 socket proxy) |
