...
| EventBridge archive replay PutEvents authz on destination bus | Closed | `InProcessTargetAuthorizerTest.eventBridgeReplayAuthorizesPutEventsOnDestinationBus` |
...
| Container path sandboxing | Closed | `PathSandbox` resolves `toRealPath()` before allowlist contains checks (`PathSandboxTest`). ECS host volumes require explicit allowed roots, Lambda hot-reload requires an allowlist, CodeBuild artifacts remain in the workspace, and S3 validates bucket names before filesystem use |
| ContainerSpec privileged / docker.sock binds (non-operator) | Closed | `ContainerSpecHardening` rejects privileged and docker.sock binds/mounts at `ContainerLifecycleManager.create` unless `operatorManaged` (EC2, EKS/k3s). Regressions: `ContainerSpecHardeningTest`. Residual: default Compose still mounts host RW `docker.sock` into Floci; use `docker-compose.socket-proxy.yml` (D.2) |
| Optional Docker socket proxy (create/start/stop/inspect allowlist) | Closed (opt-in) | `docker-compose.socket-proxy.yml` mounts sock only into `tecnativa/docker-socket-proxy`, sets `FLOCI_DOCKER_DOCKER_HOST=tcp://docker-socket-proxy:2375`, allowlists `CONTAINERS`/`POST`/`ALLOW_START`/`ALLOW_STOP` (+ `IMAGES`/`NETWORKS`). Regression: `DockerSocketProxyComposeTest`. Residual: default Compose still mounts RW `docker.sock` into Floci (LAN/internet exposure = host root); proxy does not block privileged/host-path create bodies (D.1) |
| Outbound private-address HTTP destinations | Closed | `OutboundUrlGuard` pin-connect (resolve once, validate, connect to pinned IP with Host/SNI, `Redirect.NEVER`) on SNS HTTP(S), API Gateway HTTP proxy integrations (REST and WebSocket after stage-var substitution), HTTP API JWT JWKS/OIDC fetches, and ALB create-time checks. Production paths use `AuthPosture.egressBlock` (no fail-open `OutboundUrlGuard(false,...)` defaults). Regressions: `SafeEgressRebindDeniedTest`, `OutboundUrlGuardTest`, `HttpProxyInvokerTest`, `JwtAuthorizerVerifierTest`, `WebSocketIntegrationInvokerSubstitutionTest` |
| ELBv2 instance-target egress pin | Closed | Instance targets that are not a registered EC2 instance use the same `OutboundUrlGuard` path as IP targets (`ElbV2InstanceTargetEgressDeniedTest`) |
| EKS token webhook `system:masters` under CTF | Closed | IAM enforcement maps accepted tokens to `system:authenticated` only (`EksTokenWebhookControllerTest.ctfIamEnforcementNarrowsAwayFromSystemMasters`) |
| IoT MQTT fan-out `iot:Receive` | Closed | `IotMqttBrokerService.fanOut` requires Receive before deliver (`IotMqttBrokerServiceTest.fanOutReceiveRequiresIotReceiveAction`) |
| IoT topic-rule MQTT republish Publish | Closed | `authorizeIotRepublish` (`InProcessTargetAuthorizerTest.iotRepublishAuthorizesPublishOnTopic`) |
| Neptune Gremlin WS handshake bind | Closed | Under IAM enforcement, upgrade requires AWS4 Authorization with a known access key (`NeptuneHandshakeAuthenticatorTest`) |
| ElastiCache Memcached AuthProxy | Closed | Host endpoint is proxy port; under IAM enforcement ASCII `auth <token>` + SigV4 (`ElastiCacheMemcachedServiceTest.createClusterPublishesProxyPortNotContainerPort`, `ElastiCacheMemcachedAuthProxyTest`) |
| SNS HTTP(S) subscription InProcess IAM | Residual (High) | Egress-only via `OutboundUrlGuard` (no AWS destination principal for arbitrary URLs). Contract locked by `SnsHttpSubscriptionEgressOnlyContractTest` |
...
| HTTP API JWT JWKS / OIDC fetch SSRF | Closed | `JwtAuthorizerVerifier` pin-connects JWKS/OIDC GETs via `OutboundUrlGuard.sendPinned` (`JwtAuthorizerVerifierTest`, `SafeEgressRebindDeniedTest`) |
