...
/**
 * Verified presigned POST is denied when bucket policy explicitly denies {@code s3:PutObject}
 * for the signing principal.
 */
@QuarkusTest
@TestProfile(CtfComposeParityProfile.class)
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
@Tag("security-regression")
class S3PresignedPostBucketPolicyDenyIntegrationTest {

    private static final String ACCOUNT = "000000000000";
    private static final String REGION = "us-east-1";
    private static final String BUCKET = "presign-post-bucket-policy-deny";
    private static final String KEY = "uploads/bucket-deny-post.txt";
    private static final String USER = "presign-post-bucket-policy-player";
    ...
    void provision() throws Exception {
        ...
        String identityPolicy = """
            {"Version":"2012-10-17","Statement":[
              {"Effect":"Allow","Action":"s3:PutObject","Resource":"%s"}
            ]}""".formatted(objectArn);
        signedIamFormPost("Action=PutUserPolicy"
                + "&UserName=" + USER
                + "&PolicyName=PresignPostBucketPolicyDenyAllow"
        ...
        String bucketPolicy = """
            {"Version":"2012-10-17","Statement":[
              {"Effect":"Deny","Principal":{"AWS":"%s"},
               "Action":"s3:PutObject","Resource":"%s"}
            ]}""".formatted(userArn, objectArn);
        s3Service.putBucketPolicy(BUCKET, bucketPolicy);
    }

    @Test
    void verifiedPresignedPostDeniedByBucketPolicy() throws Exception {
        ...
        given()
                ...
                .multiPart("file", "bucket-deny-post.txt", "blocked".getBytes(StandardCharsets.UTF_8), "text/plain")
        .when()
                .post("/" + BUCKET)
        .then()
                .statusCode(403)
