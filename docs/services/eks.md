...
With `FLOCI_SERVICES_IAM_ENFORCEMENT_ENABLED=true`, `CreateCluster` `roleArn` and nodegroup `nodeRole` require `iam:PassRole` (`eks.amazonaws.com`). CloudFormation `AWS::EKS::Cluster` inherits via `EksService`. Accepted tokens map to `system:authenticated` only (not `system:masters` / cluster-admin). Lab defaults without IAM enforcement still map to `system:masters`. `EksTokenValidator` verifies SigV4 on the embedded presigned STS `GetCallerIdentity` URL inside `k8s-aws-v1.*` tokens (upstream accepted any token as cluster-admin). Rejects expired URLs, unknown access keys, and tampered signatures. SigV4a presigned URLs verify when EC public keys are configured under `FLOCI_CTF_SIGV4A_*`. Regressions: `EksClusterRolePassRoleTest`, `EksTokenSigV4IntegrationTest`, `EksTokenWebhookControllerTest.ctfIamEnforcementNarrowsAwayFromSystemMasters`. See [AGENTS.md](../../AGENTS.md) and [iam.md](./iam.md#ctf-hardening).
