...
- **Federated crypto under strict IAM:** When IAM strict mode is on (Compose CTF), `AuthPosture` requires federated token crypto even if `application.yml` leaves `validate-federated-tokens` at the lab default `false`. Unsigned / `alg=none` web identity tokens return `InvalidIdentityToken` (`StsWebIdentityStrictUnsignedIntegrationTest`). YAML defaults are not flipped globally (B.4 owns CTF profile).
