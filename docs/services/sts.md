...
- **Federated crypto under strict IAM:** When IAM strict mode is on (Compose CTF / profile `ctf`), `AuthPosture` requires federated token crypto. Profile `ctf` also sets `validate-federated-tokens: true`. Main `application.yml` lab default stays `false`. Unsigned / `alg=none` web identity tokens return `InvalidIdentityToken` (`StsWebIdentityStrictUnsignedIntegrationTest`).
