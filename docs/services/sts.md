...
- **SAML XML signature verification:** When validation is enabled and `FLOCI_CTF_FEDERATED_SAML_SIGNING_CERT_PEM` or per-provider `FLOCI_CTF_FEDERATED_SAML_SIGNING_CERTS__*` is set, SAML assertions must carry a valid enveloped XML signature from a pinned trust anchor (Apache Santuario). The SAML XML parser rejects DOCTYPE declarations and external entities. Structural checks still require a non-trivial `SignatureValue` when no cert is configured.
