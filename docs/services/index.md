...
Floci emulates 57 AWS services on a single port (`4566`). All services use the real AWS wire protocol, your existing AWS CLI commands and SDK clients work without modification.
...
| EMR | `POST /` + `X-Amz-Target: ElasticMapReduce.*` | JSON 1.1 | 24 |
