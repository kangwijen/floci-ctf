...
- Task-definition host volumes are rejected unless `FLOCI_CTF_ECS_ALLOW_HOST_VOLUMES=true` and the host path is under `FLOCI_CTF_ECS_ALLOWED_HOST_SOURCE_PATHS`. Allowed-root checks use `PathSandbox` real-path resolution (symlink escape denied). Docker socket paths remain rejected at register time and again at container create via `ContainerSpecHardening` for non-operator specs.
