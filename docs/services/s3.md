...
Under Compose CTF hardening, IAM enforcement and SigV4 remain the primary S3 auth path. Upstream `FLOCI_SERVICES_S3_ENFORCE_AUTH` (`floci.services.s3.enforce-auth`, default `false`) is a separate service-level gate for unsigned reads via public ACLs or bucket policy. It does not replace Compose IAM/SigV4.
