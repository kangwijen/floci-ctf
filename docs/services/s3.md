...
Presigned POST uses policy and `x-amz-signature` form fields; strict mode rejects missing fields and expired policies (`S3PresignedPostCtfIntegrationTest`). Built-in `PreSignedUrlGenerator` signs with the operator root AKIA pair for operator tooling.
