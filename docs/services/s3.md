...
|---|---|
| `PutBucketLogging` | `PUT /{bucket}?logging` with `BucketLoggingStatus` XML (`TargetBucket`, optional `TargetPrefix`) |
...
When logging is enabled on a source bucket, `GET`, `PUT`, `DELETE`, and list (`GET` bucket) operations append newline-delimited records to the target bucket using the [AWS S3 server access log format](https://docs.aws.amazon.com/AmazonS3/latest/userguide/LogFormat.html). Log objects are rolled hourly at `{TargetPrefix}{yyyy-MM-dd-HH}-floci-s3-access.log`. Internal log delivery does not recurse into the target bucket's own logging configuration.
