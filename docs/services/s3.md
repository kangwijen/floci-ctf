...
Presigned POST uses policy and `x-amz-signature` form fields; strict mode rejects missing fields, expired policies, unknown condition operators, malformed condition arrays, and SigV4a (`AWS4-ECDSA-P256-SHA256`) algorithms with explicit errors (`S3PresignedPostIntegrationTest`). Built-in `PreSignedUrlGenerator` signs with the operator root AKIA pair for operator tooling.
