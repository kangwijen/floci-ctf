...
Under IAM enforcement and `FLOCI_AUTH_VALIDATE_SIGNATURES=true`, presigned GET/PUT URLs must carry a valid SigV4 query signature from a registered IAM key or the operator root pair. Presigned POST requires policy and signature form fields under strict mode; expired policies are rejected. See [IAM CTF hardening](iam.md#ctf-hardening).
