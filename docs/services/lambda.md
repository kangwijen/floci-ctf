...
Ports in the Runtime API range are probed on the host before assignment. When the range is busy, Floci can assign an ephemeral port instead so invocations still succeed.
...
!!! warning "CTF fork (`floci-ctf`)"
    When IAM enforcement is enabled, Lambda containers do **not** receive host `AWS_*` passthrough or `test`/`test` placeholders. `LaunchedContainerAwsEnv` sets endpoint/region baseline only. Blank `Role` on `UpdateFunctionConfiguration` is rejected (`InvalidParameterValueException`); under IAM enforcement `OperatorCredentialEnv` is never injected for blank roles. When a function has an execution role, the Lambda container credentials server on port **9171** supplies temporary role credentials. `ContainerEnvHardening` strips static keys from the runtime env. See [CTF fork](#ctf-fork).
...
- When no execution role is configured and IAM enforcement is **off**, `OperatorCredentialEnv` may inject operator root keys from `FLOCI_AUTH_ROOT_*` only (never baked-in defaults). Under IAM enforcement, blank roles never receive operator keys.
- Blank `Role` on `UpdateFunctionConfiguration` is rejected (`InvalidParameterValueException`). Regression: `LambdaBlankRoleOperatorCredentialIntegrationTest`.
...
Per [AWS Lambda function URL auth](https://docs.aws.amazon.com/lambda/latest/dg/urls-auth.html), `AuthType=NONE` allows unauthenticated HTTP access only when the function resource policy also grants `lambda:InvokeFunctionUrl` to the anonymous principal. Floci mirrors this under strict IAM: `AnonymousAccessGate` checks both `AuthType=NONE` on `/lambda-url/{urlId}` and a resource-policy Allow for `lambda:InvokeFunctionUrl`. `AuthType=AWS_IAM` always requires SigV4 (enforced in `LambdaUrlInvocationController` even when IAM strict enforcement is off). HTTP IAM evaluates the concrete function ARN resolved from the urlId store (`ResourceArnBuilder`), not `function:*`. Positive regression: `LambdaFunctionUrlNoneAuthIntegrationTest`. Negative regressions: `AnonymousAccessGateBypassIntegrationTest`, `LambdaUrlArnNotWildcardTest`, `LambdaUrlAwsIamAuthTypeTest`. See [Anonymous-access exceptions](iam.md#anonymous-access-exceptions) in the IAM service doc.
