...
Custom/third-party job workers remain ungated (residual). `CreatePipeline` / `UpdatePipeline`
`roleArn` requires `iam:PassRole` (`codepipeline.amazonaws.com`). Regression:
`CodePipelineCodeBuildDeniedTest`, `CodePipelineCreatePassRoleTest`, `InProcessTargetAuthorizerTest`.
