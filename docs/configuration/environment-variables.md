...
| `FLOCI_SERVICES_IAM_STRICT_ENFORCEMENT_ENABLED` | `false` | When `true` with enforcement enabled, deny missing auth, unregistered access keys, unknown IAM action mappings, invalid presigned URLs, AssumeRole for roles that do not exist. `AuthPosture` also coerces federated crypto and signatures-required when strict is on. Main YAML stays false; profile `ctf` / Compose set `true` |
| `FLOCI_CTF_VALIDATE_FEDERATED_TOKENS` | `false` | When `true`, STS web identity and SAML assertions must be structurally valid, JWT `exp` must be in the future, `alg=none` is rejected, SAML must contain a `Signature` element, JWT crypto is verified when signing keys are configured below, and SAML XML signatures are verified against configured trust-anchor PEMs with claims bound to the signed Assertion (XSW rejected). Profile `ctf` / Compose set `true`. Strict mode also requires federated crypto via `AuthPosture` |
...
| `FLOCI_CTF_FEDERATED_SAML_SIGNING_CERT_PEM` | _(none)_ | PEM-encoded X.509 certificate trust anchor for SAML assertion XML signature verification |
| `FLOCI_CTF_FEDERATED_SAML_SIGNING_CERTS__<provider_name>` | _(none)_ | Per SAML provider signing certificate PEM (for example `FLOCI_CTF_FEDERATED_SAML_SIGNING_CERTS__CorpIdP`) |
...
| `FLOCI_CTF_BLOCK_PRIVATE_OUTBOUND_URLS` | `false` (Compose `true`) | Drives `AuthPosture.egressBlock`. When true, outbound HTTP destinations for SNS HTTP(S), API Gateway HTTP integrations (including WebSocket HTTP_PROXY after stage-var substitution), HTTP API JWT JWKS/OIDC fetches, and ALB IP or hostname targets must resolve to public addresses. Delivery uses pin-connect (resolve once, validate, connect to that IP with Host/SNI, no redirects). Literal private IPs and metadata hostnames are rejected. Unresolved public hostnames are allowed at create or subscribe time and fail closed when pin-connect resolves a non-public address at delivery |
...
| Variable | Default | Description |
|---|---|---|
| `FLOCI_AUTH_VALIDATE_SIGNATURES` | `false` | When `true`, verifies SigV4 `Authorization` headers on inbound AWS API requests and validates S3 pre-signed URL signatures (IAM access key secret or operator root pair). Also gates `RequestContext.accessKeyId` for PassRole until verification succeeds. `true` in this fork's Compose profile. `AuthPosture.signaturesRequired` is also true under strict even when this knob stays false (SigV4 filter still follows this knob until lab profiles migrate) |
...
| Variable | Default | Description |
|---|---|---|
| `FLOCI_DOCKER_DOCKER_HOST` | `unix:///var/run/docker.sock` | Docker daemon socket path or TCP address. On Windows, when unset and the default unix socket is configured, Floci auto-falls back to `npipe:////./pipe/docker_engine` unless host `DOCKER_HOST` is set. Optional Compose override `docker-compose.socket-proxy.yml` sets `tcp://docker-socket-proxy:2375` so Floci never mounts the host sock |
