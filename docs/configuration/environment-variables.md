...
| `FLOCI_CTF_VALIDATE_FEDERATED_TOKENS` | `false` | When `true`, STS web identity and SAML assertions must be structurally valid, JWT `exp` must be in the future, `alg=none` is rejected, SAML must contain a `Signature` element, JWT crypto is verified when signing keys are configured below, and SAML XML signatures are verified against configured trust-anchor PEMs |
