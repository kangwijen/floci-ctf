...
Compose mounts the host Docker socket (`/var/run/docker.sock`) so Lambda and EC2 container runtimes can start workloads. That RW mount is host-root equivalent if Floci is exposed on a LAN or the internet. To route Docker API calls through an allowlisted `docker-socket-proxy` instead (create/start/stop/inspect subset, sock not mounted into Floci), run `docker compose -f docker-compose.yml -f docker-compose.socket-proxy.yml up` (see [Docker Compose](./docs/configuration/docker-compose.md#optional-docker-socket-proxy-api-allowlist)). For a custom `docker run`, pass the same mount or set `DOCKER_HOST` (on Windows with Docker Desktop: `npipe:////./pipe/docker_engine`), and set `QUARKUS_PROFILE=ctf` (or the equivalent `FLOCI_*` CTF knobs). Plain `docker run` without the CTF profile or those env vars is unsupported for CTF (lab YAML defaults stay permissive). Without Docker access, workloads that depend on container runtimes will not start.
...
| `FLOCI_CTF_BLOCK_PRIVATE_OUTBOUND_URLS` | YAML default `false`, Compose `true`: reject non-public destinations for SNS HTTP(S), API Gateway HTTP integrations (including WebSocket HTTP_PROXY after stage-var substitution), HTTP API JWT JWKS/OIDC fetches, and ALB IP or hostname targets |
| `FLOCI_CTF_OUTBOUND_URL_HOST_ALLOWLIST` | Optional comma-separated hostname allowlist for outbound HTTP destinations |
| `FLOCI_CTF_OUTBOUND_ALLOW_PRIVATE_ADDRESSES` | Default `false`: operator override for permitted non-public outbound destinations |
| `FLOCI_CTF_VALIDATE_FEDERATED_TOKENS` | Optional; YAML default `false`, Compose `true`. Structural JWT/SAML checks plus optional HS256/RS256 verification via `FLOCI_CTF_FEDERATED_JWT_*` |
| `FLOCI_CTF_REQUIRE_JWT_SIGNATURE_VERIFICATION` | Default `true`: HTTP API JWT authorizers require a verified signature |
| `FLOCI_CTF_API_GATEWAY_JWT_HMAC_SECRET` | Optional HS256 secret for HTTP API JWT authorizers |
| `FLOCI_CTF_REQUIRE_EKS_TOKEN_SIGV4` | Default `true`: EKS bearer tokens must carry a valid SigV4 or SigV4a signature |
| `FLOCI_CTF_ECS_ALLOW_HOST_VOLUMES` | Default `false`: ECS host source paths require this plus an allowlist |
...
Compose keeps Lambda hot-reload disabled. Enable it only with a nonempty
`FLOCI_SERVICES_LAMBDA_HOT_RELOAD_ALLOWED_PATHS` list of approved host roots.
